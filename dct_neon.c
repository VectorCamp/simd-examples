...
//into d
//REF: https://code.videolan.org/videolan/x264/-/blob/master/common/dct.c?ref_type=heads
...
{   
    ...
    {   
        ...
        /*
        printf("Row %d:\n", i);
        printf("d[%d] = %d, d[%d] = %d\n", i * 4 + 0, d[i * 4 + 0], i * 4 + 1, d[i * 4 + 1]);
        printf("d[%d] = %d, d[%d] = %d\n", i * 4 + 2, d[i * 4 + 2], i * 4 + 3, d[i * 4 + 3]);
        */
        ...
        printf("s01 = %d, s23 = %d\n", s01, s23);
        printf("d01 = %d, d23 = %d\n", d01, d23);
        printf("tmp[%d]=%d, tmp[%d]=%d, tmp[%d]=%d, tmp[%d]=%d\n", 0*4+i, tmp[0*4+i], 1*4+i, tmp[1*4+i],2*4+i, tmp[2*4+i], 3*4+i,tmp[3*4+i] );
        printf("\n");
    }
...
{   
    //hold the intermediate results
    int *tmp = (int *)aligned_alloc(16, 16 * sizeof(int));
    printf("\n");
    //phase 1
    for( int i = 0; i < 4; i++ ){
        
        int32x4_t input_row = vld1q_s32(&d[i*4]);

        //s01, s23: s01+s23 -> tmp[0] and s01-s23 ->tmp[1]
        int32x2_t sum_halves = vpadd_s32(vget_low_s32(input_row), vget_high_s32(input_row));
        tmp[0*4+i] = vaddvq_s32(input_row); //vget_lane_s32(sum_halves, 0)+vget_lane_s32(sum_halves, 1);
        tmp[1*4+i] = vget_lane_s32(sum_halves, 0)-vget_lane_s32(sum_halves, 1);
        printf("s01 = %d, s23 = %d\n", vget_lane_s32(sum_halves, 0), vget_lane_s32(sum_halves, 1));

        //d01, d23: d01-d23 -> tmp[2] and d01+d23 -> tmp[3]
        int32x2_t sub_halves = vsub_s32(vget_low_s32(input_row), vget_high_s32(input_row));
        tmp[2*4+i] = vget_lane_s32(sub_halves, 0)-vget_lane_s32(sub_halves, 1);
        tmp[3*4+i] = vget_lane_s32(sub_halves, 0)+vget_lane_s32(sub_halves, 1);
        printf("d01 = %d, d23 = %d\n", vget_lane_s32(sub_halves, 0), vget_lane_s32(sub_halves, 1));
...
int main(int argc, char **argv) {
    ...
    //seed, times & memory allignment
    srand(time(NULL));
    struct timeval tv1, tv2, tv3, tv4, diff1, diff2;


    int *d = (int *)aligned_alloc(16, 16 * sizeof(int));
    int *dd = (int *)aligned_alloc(16, 16 * sizeof(int));
    if (d == NULL || dd == NULL) {
    ...
    for (int i = 0; i < 16; i++) {
        int random_value = rand() % 256;    //integers between 0-255
        d[i] = random_value;
        dd[i] = random_value;
    }
